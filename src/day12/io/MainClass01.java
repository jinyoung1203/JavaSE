...
public class MainClass01 {
	public static void main(String[] args) throws IOException {
		...
		// c 드라이브 하위에 있는 모든 디렉토리(폴더) 및 파일을 반환한다.
		String[]list = f.list();
		
		// Calendar 객체를 현재 시스템 날짜로 생성한다
		Calendar c = Calendar.getInstance();
		
		for(String temp : list) { //파일목록 반복문 이용하여 하나씩 가져오기
			// c드라이브의 하위 디렉토리 및 파일을 새로운 File 객체로 생성한다
			File f2 = new File(f, temp);
			
			// 1970년 1월 1일 부터 현재 수정한 날짜를 밀리세컨드로 반환한다.
			...
			// 날짜포맷 만들어주는 객체
			SimpleDateFormat s = new SimpleDateFormat("yyyy년 MM월dd일 a hh:mm");
			
			// SimpleDateFormat 객체를 활용해 Calender 객체를 사용자 포맷 문자열로 반환한다.
			// Calendar 객체 getTime 메소드를 이용하여 Date 객체로 변환한다.
			String today1 = s.format(c.getTime());
			
			//isDirectory() 디렉토리 true, 파일이면 false
			if(f2.isDirectory()) {
		...
		for(String temp : list) {
			...
			long lastmod = f2.lastModified();
			c.setTimeInMillis(lastmod);
			SimpleDateFormat s = new SimpleDateFormat("yyyy년 MM월 dd일 a hh:mm");
			String today1= s.format(c.getTime());
			
			if(f2.isFile()) {
				String size = String.format("%,d" , f2.length());
				
				System.out.println(f2.getName()+"(파일) - " +size+"byte - ");
				System.out.println(today1);
